-- Create RPC function for atomic daily entry processing with stock management
-- This handles CREATE, UPDATE, DELETE operations in a single transaction

CREATE OR REPLACE FUNCTION public.process_daily_sale(
  p_operation TEXT,  -- 'create', 'update', 'delete'
  p_client_id UUID,
  p_entry_data JSONB DEFAULT NULL,  -- For create/update
  p_entry_id UUID DEFAULT NULL,  -- For update/delete
  p_old_nozzles JSONB DEFAULT NULL,  -- For update (to revert old stock)
  p_nozzle_tank_map JSONB DEFAULT NULL  -- Map of nozzle_id -> tank_id
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF p_operation = 'create' THEN
    ...
    IF p_nozzle_tank_map IS NOT NULL THEN
      FOR v_nozzle IN SELECT * FROM jsonb_array_elements(COALESCE(p_entry_data->'nozzles', '[]'::JSONB))
      LOOP
        v_tank_id := (p_nozzle_tank_map->>v_nozzle->>'id')::UUID;
  ...
  ELSIF p_operation = 'update' THEN
    ...
    IF p_old_nozzles IS NOT NULL AND p_nozzle_tank_map IS NOT NULL THEN
      ...
      FROM daily_entries WHERE id = p_entry_id;
  ...
  ELSIF p_operation = 'delete' THEN
    -- First, get the entry to restore stock
    SELECT nozzles, testing_deduction INTO p_old_nozzles, v_testing_deduction
    FROM daily_entries WHERE id = p_entry_id;
...
GRANT EXECUTE ON FUNCTION public.process_daily_sale TO authenticated;